...
function initBeams() {
  const ctn = document.getElementById('beams-bg');
  if (!ctn) return;
  ...
  const camera = new THREE.PerspectiveCamera(30, ctn.clientWidth / ctn.clientHeight, 0.1, 1000);
  camera.position.z = 50;

  const beamMaterial = new THREE.MeshStandardMaterial({
    color: 0x000000, roughness: 0.3, metalness: 0.3
  });
